...
function mapProperty(model, property, modelName, propertyName) {
    ...
    if (scalar) {
        ...
        if (property.enum) { // enum is not a scalar and should have a dedicated type
            types[typeName] = {
                values: property.enum,
                category: 'ENUM'
            };
            currentProperty.gqlType = typeName;
            currentProperty.scalar = false;
        }
    }
    ...
    if (propertyType.name === 'ModelConstructor') {
        ...
        if (union.length > 1) { // union type
        ...
        } else if (propertyType.definition) {
            types[typeName] = {
                category: 'TYPE',
                fields: {}
            }; // creating a new type
            ...
            console.log('UNKNOWN TYPE ', typeName, propertyType.modelName);
        }
    }
}
...
function mapRoot(model) {
    ...
    types.Mutation.fields[`save${utils.singularModelName(model)}`] = {
        relation: true,
        params: `obj: ${model.modelName}Input!`,
