...
***/
    ...
    _.forEach(properties, (p, key) => {
        ...
        } else if (helper.toType[p.type.name]) { // simple scalar - add prop only
...
function generateModels(models) {
    return _.map(models, model => {
        console.log('MODEL: ', model.modelName, helper.validProps(model));

        let definitions = generateDefinitions(
            model.modelName,
            helper.validProps(model)
        );
